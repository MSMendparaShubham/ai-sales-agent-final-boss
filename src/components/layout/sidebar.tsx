        ...
        <div className="flex flex-col gap-2 p-2 rounded-lg bg-white border border-slate-200/80 shadow-xs hover:border-slate-300 transition-colors">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5">
                <p className="text-xs font-semibold text-slate-800 truncate">{session?.user?.name || 'Alex Morgan'}</p>
                <span className="text-[9px] font-extrabold uppercase bg-emerald-100 text-emerald-800 border border-emerald-200 px-1.5 py-0.5 rounded">
                  Enterprise
                </span>
              </div>
              <p className="text-[10px] text-slate-500 truncate">{session?.user?.email || 'alex.morgan@intentos.ai'}</p>
            </div>
          </div>
        </div>
      </div>
